...
// TestReadonlyContextDelegatesUnknownKeys pins the half of ReadonlyContext.Value
// that its own doc claims and nothing asserted: every key but the identity one is
// handed to the embedded context.
//
// The existing probes could not see it. They assert Value(probeKey{}) == nil for a
// key nothing plants, which a Value that returned nil for everything also
// satisfies, and the one cell that does plant a value puts it ABOVE the readonly
// context, so the read is answered by the WithValue node and never enters this
// method. Replacing the delegation with `return nil` left all 90 packages green.
func TestReadonlyContextDelegatesUnknownKeys(t *testing.T) {
	parent := context.WithValue(t.Context(), wrapKey{}, "x")
	ic := icontext.NewInvocationContext(parent, icontext.InvocationContextParams{Session: (*ptrSession)(nil)})
	ro := icontext.NewReadonlyContext(ic)
	if got := ro.Value(wrapKey{}); got != "x" {
		t.Errorf("ReadonlyContext.Value(wrapKey{}) = %v, want %q — a key that is not the "+
			"identity one must reach the embedded context", got, "x")
	}
}
